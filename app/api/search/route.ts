...
const busStops     = busStopsJson     as Record<string, { name: string; routes: string[] }>;
...
export async function GET(req: NextRequest) {
  ...
  for (const [id, { name, routes }] of Object.entries(busStops)) {
    if (busResults.length >= 100) break;
    if (
      name.toLowerCase().includes(q) ||
      routes.some((r) => r.toLowerCase().startsWith(q))
    ) {
      busResults.push({ id, name, type: "BUS", lines: routes });
    }
  }
